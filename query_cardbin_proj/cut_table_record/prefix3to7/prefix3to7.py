...
#-*- coding: utf-8 -*-
'''
filename:       prefix3to7.py
description:    修改sql文件里的insert语句，智能分拆记录，由3位前导符修改成5位前导符的
'''
import os, sys


def sql_line_prefix3to7(line):      # with '\n'
    '''
    ...
    low_prefix = int(tuple_row[2][:7])
    low_suffix = tuple_row[2][7:]
    high_prefix = int(tuple_row[5][:7])
    high_suffix = tuple_row[5][7:]

    for i in range(low_prefix, high_prefix+1):
        new_list_row = list(tuple_row)
        prefix = '%07d' % i     # 注意，前导0要保留！
...
def sql_file_prefix3to7(filename, tofilename='prefix3to7.sql'):
    infile = open(filename, 'r')
    outfile = open(tofilename, 'w')
    for line in infile:
        # print repr(line);
        if len(line)>4 and line[0]=='(' and line[-3]==')' and line[-2] in (',',';'):
            outfile.write(sql_line_prefix3to7(line))
        else:
            outfile.write(line)
    infile.close()
    outfile.close()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        exit(2)
    if len(sys.argv) == 2:
        sql_file_prefix3to7(sys.argv[1])
    else:
        sql_file_prefix3to7(sys.argv[1], sys.argv[2])
