 ...
 * =====================================================================================
 *
 *       Filename:  txproj_pay.cpp
 *
 *    Description:  txproj_pay?password_pay=xxx&dealid=xxx
 *
 *        Version:  1.0
 *        Created:  2014年08月17日 19时37分30秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int main(int argc, char *argv[])
{
    try {
        ...
        const_form_iterator 
            password_pay    = cgi.getElement("password_pay"),
            dealid          = cgi.getElement("dealid"),
            end             = cgi.getElements().end();

        if (type!="" && name!="" && token!="" && id!="" &&
                password_pay!=end && dealid!=end){
            string cmd = "pay|" + dealid->getValue() + '|' + password_pay->getValue() + '|' + id + '|' + token;
